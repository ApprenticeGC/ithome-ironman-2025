...
def run_demo_command(description, issue_number, action, assignees=None):
    ...
    print(f"📝 Expected: This would set the issue status based on the action")
    print(f"⚠️  Note: This is a demo - no actual API calls will be made without valid token")
...
def main():
    ...
    run_demo_command(
        "Issue Assigned to Multiple Users",
        issue_number=124,
        action="assigned", 
        assignees=["copilot", "reviewer"]
    )
