...
def gql(query: str, vars: dict, tok: str) -> dict:
    req = urllib.request.Request(API,
        ...
        headers={"Authorization": f"bearer {tok}", "Content-Type":"application/json", "User-Agent":"assign-copilot/1.0"})
    with urllib.request.urlopen(req) as r:
        obj = json.loads(r.read().decode())
    if obj.get("errors"): sys.stderr.write(json.dumps(obj["errors"]) + "\n"); sys.exit(3)
    return obj.get("data", {})
...
def main(argv:list[str])->int:
    ...
    p.add_argument("--title", required=True); p.add_argument("--body")
    ...
    nodes = gql(SUG_Q,{"owner":args.owner,"name":args.repo},tok)["repository"]["suggestedActors"]["nodes"]
    sel = None
    pref = {"bot":"Bot","user":"User"}.get(args.assign_mode)
    if pref:
        for n in nodes:
            if n.get("__typename") == pref and "copilot" in (n.get("login") or "").lower():
                sel = n; break
    if not sel and nodes:
        sel = nodes[0]
    if not sel: sys.stderr.write("No suggestedActors found\n"); return 4
    aids = [sel.get("id")] if sel.get("id") else None
    data = gql(CRT_M,{"rid":rid,"title":args.title,"body":args.body,"aids":aids},tok)
    issue = data["createIssue"]["issue"]
    out = {"issue_number": issue["number"], "issue_url": issue["url"], "assignee_login": sel.get("login"), "assignee_type": sel.get("__typename")}
