...
def pick_bot_id(owner: str, name: str) -> str | None:
    ...
    nodes = (
        ((data.get("data") or {}).get("repository") or {}).get("suggestedActors") or {}
    ).get("nodes") or []
    for n in nodes:
...
def main(argv: list[str]) -> int:
    ...
    for pr in prs:
        ...
        if re.search(rf"RFC-{re.escape(rfc)}-\d{{2}}", title, re.IGNORECASE):
    ...
    # assign
    issue_node = (
        run_gh_json(["issue", "view", str(sel_num), "--repo", repo, "--json", "id"])
        or {}
    )
