...
def select_next(rfc: int, next_micro: int, issues: list[dict]):
    variants = [
        f"Game-RFC-{rfc:03d}-{next_micro:02d}",
        f"Game-RFC-{rfc}-{next_micro}",
        f"RFC-{rfc:03d}-{next_micro:02d}",
        f"RFC-{rfc}-{next_micro}",
    ]
    cands = []
    for it in issues:
        ...
        for rank, v in enumerate(variants):
            idx = title.find(v)
            if idx >= 0:
                cands.append((rank, idx, int(it["number"]), it))
                break
    if not cands:
        return None
...
def assign_issue_to_copilot(repo: str, issue: int) -> bool:
    ...
    try:
        ...
        if not bot:
            ...
            out = subprocess.run(
                [
                    ...
                    "graphql",
                    "-f",
                    "query=query($login:String!){ user(login:$login){ id __typename } }",
                    "-F",
                    "login=Copilot",
                ],
                capture_output=True,
        ...
        if not bot:
            ...
            return False
        issue_id = run_gh_json(
            ["issue", "view", str(issue), "--repo", repo, "--json", "id"]
        ).get("id")
...
def main():
    ...
    assign = "--assign" in sys.argv or os.environ.get("ASSIGN") == "1"
    ...
    prj = (
        run_gh_json(["pr", "view", str(pr), "--repo", repo, "--json", "body,title"])
        or {}
    )
    closed = None
    ...
    if not closed:
        ...
        issue = (
            run_gh_json(
                ["issue", "view", str(closed), "--repo", repo, "--json", "title"]
            )
            or {}
        )
        rfc, mic = parse_issue_title(issue.get("title", ""))
    if not rfc or not mic:
        print("Not a micro RFC title")
        sys.exit(0)
    nexti = mic + 1
    open_issues = (
        run_gh_json(
    ...
    if not sel:
        ...
        if not sel:
            ...
            print(
                json.dumps(
                    {
                        "message": "No next micro issue",
                        "rfc": rfc,
                        "current": mic,
                        "open_titles": [it.get("title") for it in open_issues],
                    }
                )
            )
            sys.exit(0)
